    ...
    async def get_dashboard_snapshot(self) -> dict:
        ...
        return {
            ...
            "account_info": {
                ...
                # Fields read by the dashboard frontend from account_info
                "daily_pnl":        float(metrics.get("daily_pnl",   0.0)),
                "weekly_pnl":       float(metrics.get("weekly_pnl",  0.0)),
                "monthly_pnl":      float(metrics.get("monthly_pnl", 0.0)),
                "floating_pnl":     round(sum(float(t.get("pnl", 0.0)) for t in open_trades), 2),
                "open_positions":   len(open_trades),
                "total_trades_today": int(metrics.get("total_trades_today", 0)),
            },
